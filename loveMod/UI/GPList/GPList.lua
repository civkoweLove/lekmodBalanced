...
local m_FarmerIM = InstanceManager:new( "GPInstance", "Root", Controls.FarmerStack );
...
function UpdateDisplay()
			...
			for pCity in pPlayer:Cities() do
				if (checkIfSpecialist(pPlayer, pSpecialistInfo, pCity)) then
					...
					elseif (pSpecialistInfo.Type == "SPECIALIST_MUSICIAN") then
						...
						bMusicianFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_FARMER") then
						instance = m_FarmerIM:GetInstance();
						bFarmerFound = true;
	...
	Controls.FarmerStack:SortChildren( SortFunction );
	...
	Controls.FarmerHeader:SetHide( not bFarmerFound );
	...
	Controls.FarmerStack:CalculateSize();
	Controls.FarmerStack:ReprocessAnchoring();
...
function getRateOfChange(city, specialistInfo, player)
	...
	if iGPPChange > 0 then
		...
		elseif (specialistInfo.GreatPeopleUnitClass == "UNITCLASS_FARMER") then
			iPlayerMod = iPlayerMod + player:GetGreatFarmerRateModifier();
...
-------------------------------------------------
-------------------------------------------------
function OnFarmerToggle()
	local bWasHidden = Controls.FarmerStack:IsHidden();
	Controls.FarmerStack:SetHide( not bWasHidden );
	if( bWasHidden ) then
		Controls.FarmerToggle:LocalizeAndSetText("TXT_KEY_GP_FARMER_DETAILS_COLLAPSE");
	else
		Controls.FarmerToggle:LocalizeAndSetText("TXT_KEY_GP_FARMER_DETAILS");
	end
	Controls.MainStack:CalculateSize();
	Controls.MainStack:ReprocessAnchoring();
	Controls.ScrollPanel:CalculateInternalSize();
...
Controls.FarmerToggle:RegisterCallback( Mouse.eLClick, OnFarmerToggle );
