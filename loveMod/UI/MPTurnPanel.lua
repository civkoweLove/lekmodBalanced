...
function OnEndTurnTimerUpdate(percentComplete)
	...
	local maxTime = getEndTurnTimerLength()
	...
	if percentComplete < 0.1 then
		HalfTimeNotification = true	
	elseif (HalfTimeNotification and Game.GetGameTurn() > 0 and 2 * timeLeft <= maxTime + 1) then
		local pPlayer = GetPlayer();		
		local pTeam = Teams[pPlayer:GetTeam()];
		if pTeam:GetAtWarCount() > 0 then
			pPlayer:AddNotification( NotificationTypes.NOTIFICATION_GENERIC,
			Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_HALF_TIME"),
			Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_SUMMARY_HALF_TIME"),
			-1, -1, -1 )
