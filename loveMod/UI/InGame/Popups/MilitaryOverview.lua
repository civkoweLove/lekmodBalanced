...
function UpdateScreen()
	...
	local iSupplyUnits = pPlayer:GetNumUnits() - pPlayer:GetNumInternationalTradeRoutesUsed();
    Controls.SupplyCapValue:SetText(        pPlayer:GetNumUnitsSupplied() );
    Controls.SupplyUseValue:SetText(        iSupplyUnits );
    
    local iSupplyDeficit = pPlayer:GetNumUnitsOutOfSupply();
    local bInDeficit = (iSupplyDeficit ~= 0 );
    
    if( not bInDeficit ) then
		Controls.SupplyRemainingValue:SetText( pPlayer:GetNumUnitsSupplied() - iSupplyUnits );
    else
