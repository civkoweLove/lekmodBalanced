...
g_HBGameType = GameTypes.GAME_NETWORK_MULTIPLAYER;
g_LastSaveTurn = -1;
...
------------------------------------------------------------------------------------
function OnEndTurnEnterPressed()
	if UI.CanEndTurn() then
...
LuaEvents.OnEndTurnClicked.Add( OnEndTurnEnterPressed );
----------------------------------------------------------------------------------------
...
function ReloadFix()
	...
	print("Checking if all players ready....");

	local iActivePlayer = Game.GetActivePlayer();

	if not Network.HasSentNetTurnComplete() then
		playersWaiting = 1;
	end
	...
	--make sure all players have hit end turn
	if(playersWaiting == 0) then
...
LuaEvents.ReloadFix.Add(ReloadFix)
Events.RemotePlayerTurnEnd.Add( ReloadFix );
