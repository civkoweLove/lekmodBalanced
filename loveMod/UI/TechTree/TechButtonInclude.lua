...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize, startingButtonNum )
	...
	-- city range
	for row in GameInfo.Technology_CityRange( thisTechType ) do
		if row.TechType == techType then
			local toolTip = ""
			
			if (row.Range or 0) > 0 then
				toolTip = "Increases range of Cities to " .. row.Range
			end
			
			if (row.IndirectFire or false) then
				toolTip = toolTip .. ". Grants Cities Indirect Fire"
			end
