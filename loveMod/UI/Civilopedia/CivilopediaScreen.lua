...
local g_ImprovingTechManager = InstanceManager:new( "ImprovingTechInstance", "ImprovingTechButton", Controls.ImprovingTechInnerFrame );
...
--------------------------------------------------------------------------------
			...
			g_ImprovingTechManager:ResetInstances()
			buttonAdded = 0
			local techsUsed = {}
			for row in GameInfo.Improvement_TechFreshWaterYieldChanges( condition ) do
				if not hasValue(techsUsed, row.TechType) then
					table.insert(techsUsed, row.TechType)
				end
			end
			for row in GameInfo.Improvement_TechNoFreshWaterYieldChanges( condition ) do
				if not hasValue(techsUsed, row.TechType) then
			...
			for i, TechType in ipairs(techsUsed) do
				local thisImprovingInstance = g_ImprovingTechManager:GetInstance();
				if thisImprovingInstance then
					local improvingTech = GameInfo.Technologies[TechType];
					local textureOffset, textureSheet = IconLookup( improvingTech.PortraitIndex, buttonSize, improvingTech.IconAtlas );				
					if textureOffset == nil then
						textureSheet = defaultErrorTextureSheet;
						textureOffset = nullOffset;
					end	
					UpdateSmallButton( buttonAdded, thisImprovingInstance.ImprovingTechImage, thisImprovingInstance.ImprovingTechButton, textureSheet, textureOffset, CategoryTech, Locale.ConvertTextKey( improvingTech.Description ), improvingTech.ID );
					buttonAdded = buttonAdded + 1;	
				end			
			end
			UpdateButtonFrame( buttonAdded, Controls.ImprovingTechInnerFrame, Controls.ImprovingTechFrame );
--------------------------------------------------------------------------------
			-- City Yield
			local numYields = 0;
			local yieldString = "";
...
function ClearArticle()
	...
	Controls.ImprovingTechFrame:SetHide( true );
