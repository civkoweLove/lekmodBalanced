...
function NZ_Team_Met(playerMetID, playerID)
	...
	if (Players[playerID]):GetCivilizationType() == civID then
		id = playerID
		player = Players[playerID]
		playerMet = Players[playerMetID]
	elseif (Players[playerMetID]):GetCivilizationType() == civID then
		id = playerMetID
		player = Players[playerMetID]
		playerMet = Players[playerID]
	else
		return
	end
	...
	if random == 1 then
		...
		if player:IsHuman() and player:GetCivilizationType() == civID and Game.GetActivePlayer() == id then
